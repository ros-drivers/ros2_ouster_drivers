...
public:
  ...
  LidarScan(size_t w, size_t h)
      : w{static_cast<std::ptrdiff_t>(w)}, h{static_cast<std::ptrdiff_t>(h)},
        data{w * h, N_FIELDS}, headers{w, BlockHeader{ts_t{0}, 0, 0}} {}
  ...
  Eigen::Map<const data_t, Eigen::Unaligned, DynStride>
  block(size_t m_id) const {
...
inline XYZLut make_xyz_lut(const sensor::sensor_info &sensor) {
  return make_xyz_lut(sensor.format.columns_per_frame,
                      ...
                      sensor.lidar_origin_to_beam_origin_mm,

 ...
 */
inline std::uint32_t toProcMask(const std::string &mask_str) {
 ...
 */
inline std::unique_ptr<ros2_ouster::DataProcessorInterface>
 ...
 */
inline std::unique_ptr<ros2_ouster::DataProcessorInterface>
createIMUProcessor(const rclcpp_lifecycle::LifecycleNode::SharedPtr node,
                   const ouster::sensor::sensor_info &mdata,
                   const std::string &frame, const rclcpp::QoS &qos,
...
createProcessors(
    const rclcpp_lifecycle::LifecycleNode::SharedPtr node,
    const ouster::sensor::sensor_info &mdata, const std::string &imu_frame,
    const std::string &laser_frame, const rclcpp::QoS &qos,
    const ouster::sensor::packet_format &pf,
    std::shared_ptr<sensor::FullRotationAccumulator> fullRotationAccumulator,
  ...
  if ((mask & ros2_ouster::PROC_IMG) == ros2_ouster::PROC_IMG) {
    data_processors.insert(
        std::pair<ouster::sensor::client_state,
                  std::unique_ptr<ros2_ouster::DataProcessorInterface>>(
            ouster::sensor::client_state::LIDAR_DATA,
            createImageProcessor(node, mdata, laser_frame, qos, pf,
                                 fullRotationAccumulator)));
  }
  ...
  if ((mask & ros2_ouster::PROC_PCL) == ros2_ouster::PROC_PCL) {
    data_processors.insert(
        std::pair<ouster::sensor::client_state,
                  std::unique_ptr<ros2_ouster::DataProcessorInterface>>(
            ouster::sensor::client_state::LIDAR_DATA,
            createPointcloudProcessor(node, mdata, laser_frame, qos, pf,
                                      fullRotationAccumulator)));
  }
  ...
  if ((mask & ros2_ouster::PROC_IMU) == ros2_ouster::PROC_IMU) {
    data_processors.insert(
        std::pair<ouster::sensor::client_state,
                  std::unique_ptr<ros2_ouster::DataProcessorInterface>>(
            ouster::sensor::client_state::IMU_DATA,
            createIMUProcessor(node, mdata, imu_frame, qos, pf)));
  }
  ...
  if ((mask & ros2_ouster::PROC_SCAN) == ros2_ouster::PROC_SCAN) {
    data_processors.insert(
        std::pair<ouster::sensor::client_state,
                  std::unique_ptr<ros2_ouster::DataProcessorInterface>>(
            ouster::sensor::client_state::LIDAR_DATA,
            createScanProcessor(node, mdata, laser_frame, qos, pf,
                                fullRotationAccumulator)));
  }
